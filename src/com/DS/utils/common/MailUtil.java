...
/***
 * 
 * @author jeff 发送邮件的工具类 发送失败的原因很常是因为被判定成垃圾邮件(最好接收方和发送方都是同种类型的邮箱)
...
public class MailUtil {
	...
	public static void initConfig(String mailAccount) {
		...
		myEmailAccount = config.getParameter1(); // p.get("myEmailAccount");
		myEmailPassword = config.getParameter2(); // p.get("myEmailPassword");
	...
	public static MimeMessage createMimeMessage(Session session, MailBean mail) throws Exception {
		...
		// InternetAddress(receiveMailAccount, mail.getReceiveName(), "UTF-8"));
